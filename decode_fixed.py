...
data = open('F:/Projects/nesrecomp/banks/bank15.bin','rb').read()
BASE = 0xC000

def dis(start_nes, count=60, label=None):
    if label:
        print(f"\n=== {label} (${start_nes:04X}) ===")
    pc = start_nes - BASE
    for _ in range(count):
        ...
        if op not in OPCODES:
            print(f"  {BASE+pc:04X}: {op:02X}        ??? ${op:02X}")
            pc += 1
            continue
        mn, sz = OPCODES[op]
        raw = data[pc:pc+sz]
        ...
        pc += sz
        if op in (0x60, 0x40): break
        if op == 0x4C: break

import sys
addr = int(sys.argv[1], 16) if len(sys.argv) > 1 else 0xDA6A
dis(addr, 80, f"${addr:04X}")
