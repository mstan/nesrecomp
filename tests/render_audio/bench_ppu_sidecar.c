...
int hdpack_active(void) { return s_hd_record; }
int hdpack_scale(void) { return 1; }
HdPixel *hdpack_pixels(void) { return s_hd_record ? s_pixels : NULL; }
int hdpack_recording(void) { return s_hd_record; }
void hdpack_frame_begin(void) { memset(s_pixels, 0, sizeof(s_pixels)); }
...
static uint32_t fnv1a_update(uint32_t h, const void *data, size_t len) { const uint8_t *p = (const uint8_t *)data; for (size_t i = 0; i < len; i++) h = (h ^ p[i]) * 16777619u; return h; }
static uint32_t hash_fb(uint32_t *fb) { return fnv1a_update(2166136261u, fb, (size_t)g_render_width * 240u * 4u); }

static void init_state(int wide, int hd) {
    memset(g_chr_ram, 0, sizeof(g_chr_ram)); memset(g_ppu_oam, 0xF8, sizeof(g_ppu_oam)); memset(g_ppu_nt, 0, sizeof(g_ppu_nt)); memset(g_ppu_pal, 0, sizeof(g_ppu_pal)); memset(g_oam_x16, 0, sizeof(g_oam_x16)); memset(s_pixels, 0, sizeof(s_pixels));
    ...
    g_ppuctrl = 0x10; g_ppumask = 0x1E; g_ppustatus = 0; g_ppuscroll_x = 5; g_ppuscroll_y = 7;
    g_ppuctrl_hud = 0; g_ppuscroll_x_hud = 0; g_ppuscroll_y_hud = 0; g_spr0_split_active = 0; g_zapper_enabled = 0; g_ws_oam_sidecar = 0;
    g_frame_count = 10; g_ppuaddr = 0; s_mirroring = 2; s_hd_record = hd;
    if (wide) { g_render_width = 512; g_widescreen_left = 128; g_widescreen_right = 128; g_ws_eff_left = 128; g_ws_eff_right = 128; }
    else { g_render_width = 256; g_widescreen_left = 0; g_widescreen_right = 0; g_ws_eff_left = -1; g_ws_eff_right = -1; }
    g_ppu_oam[0] = 20; g_ppu_oam[1] = 4; g_ppu_oam[2] = 0; g_ppu_oam[3] = 30;
    g_ppu_oam[4] = 31; g_ppu_oam[5] = 6; g_ppu_oam[6] = 0x21; g_ppu_oam[7] = 44;
}

static double bench_case(const char *name, int wide, int hd, int frames, uint32_t *hash) {
    static uint32_t fb[512 * 240];
    init_state(wide, hd);
    for (int i = 0; i < 512 * 240; i++) fb[i] = 0x12345678u;
    for (int i = 0; i < 12; i++) ppu_render_frame(fb);
    double t0 = now_ms();
    for (int i = 0; i < frames; i++) ppu_render_frame(fb);
    double elapsed = now_ms() - t0;
    *hash = hash_fb(fb) ^ (uint32_t)ppu_renderer_background_opaque(30, 20);
    printf("%s ms=%.3f hash=%08x\n", name, elapsed, *hash);
    return elapsed;
}

int main(void) {
    uint32_t h;
    bench_case("classic", 0, 0, 2600, &h);
    bench_case("widescreen_full", 1, 0, 1600, &h);
    bench_case("hdpack_record", 0, 1, 1800, &h);
    return 0;
}
