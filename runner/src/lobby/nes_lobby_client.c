...
int  nes_lobby_connect(const char *ws_url) { (void)ws_url; return -1; }
void nes_lobby_disconnect(void) {}
int  nes_lobby_connected(void) { return 0; }
void nes_lobby_set_display_name(const char *name) { (void)name; }
const char *nes_lobby_display_name(void) { return ""; }
const char *nes_lobby_player_id(void) { return ""; }
void nes_lobby_pump(void) {}
void nes_lobby_request_list(void) {}
int  nes_lobby_list_count(void) { return 0; }
int  nes_lobby_list_get(int index, NesLobbyRow *out) { (void)index; (void)out; return 0; }
void nes_lobby_set_game_identity(const char *a, const char *b) { (void)a; (void)b; }
const char *nes_lobby_game_version(void) { return NES_GAME_VERSION; }
int  nes_lobby_create(const char *a, const char *b, const char *c, const char *d,
                       const char *e, const NesLobbyMatchCaps *f)
{ (void)a; (void)b; (void)c; (void)d; (void)e; (void)f; return -1; }
int  nes_lobby_join(const char *a, const char *b, const char *c)
{ (void)a; (void)b; (void)c; return -1; }
int  nes_lobby_leave(void) { return -1; }
int  nes_lobby_in_lobby(void) { return 0; }
int  nes_lobby_is_host(void) { return 0; }
const NesLobbyJoinInfo *nes_lobby_join_info(void)
{
    static NesLobbyJoinInfo z;
    return &z;
}
const NesLobbyMatchCaps *nes_lobby_match_caps(void)
{
    static NesLobbyMatchCaps z;
    return &z;
}
int  nes_lobby_set_match_caps(const NesLobbyMatchCaps *c) { (void)c; return -1; }
int  nes_lobby_member_count(void) { return 0; }
int  nes_lobby_member_get(int index, NesLobbyMember *out) { (void)index; (void)out; return 0; }
int  nes_lobby_local_ready(void) { return 0; }
int  nes_lobby_all_ready(void) { return 0; }
int  nes_lobby_set_ready(int ready) { (void)ready; return -1; }
int  nes_lobby_request_start(const NesLobbyMatchCaps *c) { (void)c; return -1; }
int  nes_lobby_launch_pending(void) { return 0; }
...
typedef struct {
    ...
    NesLobbyRow list[NES_LOBBY_MAX_LIST];
    int list_count;
    int in_lobby;
    int is_host;
    char my_bind[NES_LOBBY_ENDPOINT_LEN];
    char filter_game_name[NES_LOBBY_NAME_LEN];
    char filter_game_version[NES_LOBBY_VERSION_LEN];
    NesLobbyJoinInfo join;
    NesLobbyMember members[NES_LOBBY_MAX_MEMBERS];
    int member_count;
    int local_ready;
    int all_ready;
    int launch_pending;
    NesLobbyMatchCaps match_caps;
...
static LobbyClient g_lc = {
    .fd = -1,
    .filter_game_version = NES_GAME_VERSION,
};
...
static void match_caps_clear(NesLobbyMatchCaps *c)
{
    if (!c) return;
    memset(c, 0, sizeof(*c));
    c->input_delay = 2;
...
static void parse_match_caps_object(const char *obj, NesLobbyMatchCaps *out);
static void ingest_match_caps_from_json(const char *json);
static int append_match_caps_json(char *dst, size_t dst_cap, const NesLobbyMatchCaps *caps);

const char *nes_lobby_default_url(void)
{
    const char *e = getenv("NES_NET_LOBBY_URL");
...
static void parse_slots_array(const char *json)
{
    ...
    ++p;
    while (*p && n < NES_LOBBY_MAX_MEMBERS) {
        const char *obj;
        while (*p && *p != '{') {
            if (*p == ']') {
                g_lc.member_count = n;
...
int nes_lobby_connect(const char *ws_url)
{
...
#endif
    if (parse_ws_url(ws_url ? ws_url : nes_lobby_default_url(), g_lc.host, sizeof(g_lc.host),
                                  ...
                                  const char *game_version)
{
    ...
    if (game_version && game_version[0]) {
        ...
        strncpy(g_lc.filter_game_version, NES_GAME_VERSION,
                sizeof(g_lc.filter_game_version) - 1);
        g_lc.filter_game_version[sizeof(g_lc.filter_game_version) - 1] = '\0';
    }
}
...
int nes_lobby_create(const char *name, const char *game_name,
                     ...
                     const char *host_bind, const NesLobbyMatchCaps *match_caps)
{
    char msg[1536];
    char caps_json[512];
    const char *gn;
    ...
    if (game_name && game_name[0])
        nes_lobby_set_game_identity(game_name, gv);
...
int nes_lobby_join(const char *lobby_id, const char *password, const char *guest_bind)
{
    char msg[1024];
    const char *gn;
    const char *gv;
    if (!nes_lobby_connected() || !lobby_id) {
        return -1;
    }
...
int nes_lobby_set_match_caps(const NesLobbyMatchCaps *caps)
{
    ...
    char caps_json[512];
    int n;
    if (!nes_lobby_connected() || !g_lc.in_lobby || !g_lc.is_host || !caps || !caps->valid)
        return -1;
...
{
    char msg[64];
    if (!nes_lobby_connected() || !g_lc.in_lobby) {
        return -1;
    }
...
int nes_lobby_request_start(const NesLobbyMatchCaps *match_caps)
{
    ...
    char caps_json[512];
    int n;
    if (!nes_lobby_connected() || !g_lc.in_lobby || !g_lc.is_host) {
        return -1;
    }
    caps_json[0] = '\0';
    if (match_caps && match_caps->valid) {
...
int nes_lobby_send_signal(int type, int flag, const char *text)
{
    ...
    snprintf(msg, sizeof(msg),
             ...
             lid, type, flag, esc);
    /* Write immediately â€” ICE candidates arrive in bursts larger than pending_tx. */
