...
|------|---------|-------------|------------------------|
| 1 Instruction semantics | 3 STRONG | **stable unofficial opcodes IMPLEMENTED** (SLO/RLA/SRE/RRA/DCP/ISC RMW combos + ANC/ALR/ARR/AXS + the prior LAX/SAX/NOP*); only the unstable set (KIL/XAA/SHA/SHX/SHY/TAS/LAS) remains sized-NOP. `JMP($xxFF)` page-wrap handled (`nes_read16_jmpbug`). Decoupled from discovery (`mn_finder_illegal`): the finder treats unofficials like illegal so implementing them does NOT change function discovery — verified all 4 games regen byte-exact (incl. MM3, which broke on `label_FEAA` until the finder fix) | unstable illegals (no real game uses them — leave as sized-NOP) |
| 2 Cycle/timing | 3 STRONG | cross-title cycle_compare drift: **SMB 0.51 / Zelda 0.41 / MM3 0.37 cyc/frame** — all dominated by frame-len 29781 vs 29780.5; model holds NROM/MMC1/MMC3. **DMC DMA cycle-steal now charged to the budget** (4 cyc/fetch) — adds the missing CPU-time loss on DMC-active frames | alternating frame budget + monotonic `g_cpu_cycles` (deferred, 0.0017% — owner-deprioritized); dynamic penalties |
...
| 4 Memory/MMIO | 3 STRONG | CPU-RAM zero game-logic divergence (SMB/Zelda/MM3); **PPU-mem MEASURED across all 3 mappers** (SMB 99.84%; Zelda NT byte-exact + palette exact bar 4 mirror-fold slots + 4 anim-phase OAM bytes ~99.7%; MM3 NT+OAM byte-exact, palette diverges only in the boot-cadence window — title framebuf 1.0) via external Mesen-Lua; **open-bus: $2002 low-5 NOW MODELED** (PPU I/O latch refreshed on PPU writes + $2004/$2007 reads; $2002 returns status[7:5] | latch[4:0]) — the old "hangs SMB RESET" landmine no longer applies after the $2002 rewrite; unmapped $4020-$5FFF + write-only APU regs return CPU open bus | unmapped-read open-bus oracle-test ROM (polish) |
...
| 5b Audio/APU | 3 STRONG | reg-stream BIT-IDENTICAL ✓; rhythm 0.99 ✓; timbre ✓; **Option B sample-accurate engine LANDED** (all timers+DMC+frame seq cycle-driven; audio no-regression, self-diff L1 0.057, vs-oracle 0.28<old 0.36); residual = mid-band tilt; **DMC DMA cycle-steal NOW MODELED** (4 CPU cycles charged to the frame budget per DMC sample-byte fetch; all 4 games 0 misses + non-DMC scenes byte-exact) | windowed-sinc decimator (polish) |
| 5c Input (peripherals) | 3 STRONG | $4016/$4017 read protocol characterized: strobe-reload, MSB-first 8-bit serial, 0x40 open-bus upper bits; FIXED — shift register now fills with 1s so reads past the 8 button bits return D0=1 (standard controller) instead of 0. Functionally validated by interactive play (SMB/Zelda/MM3/Faxanadu) + zero game-logic RAM divergence | DMC-DMA-during-controller-read bit-drop corruption (rides the DMC capstone); open-bus upper bits exact (0x40 approx) |
